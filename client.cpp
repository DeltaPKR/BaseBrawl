...
// Game data (all received from server)
struct Unit { float x; int hp, maxHp, type; };

struct Projectile 
{
    float x, y, vx, vy;
    float life;          // seconds until despawn
    sf::Color col;
...
int main()
{
    ...
    // Client-side turret timers for projectile VFX (mirror server fire rates)
    float laneTurrCD[3] = { 0.f, 0.f, 0.f };
    float myMainTurrCD = 0.f;
    float enMainTurrCD = 0.f;
    std::vector<Projectile> projectiles;

    // Perspective team colors: always blue=me, red=enemy
    ...
    while (window.isOpen()) 
    {
        ...
        // Derive once, used in both simulation and render
        bool iAmP1 = (g.myID == 1);

        // Projectile VFX simulation
        // Runs client-side at matching fire rates — purely visual, no gameplay impact.
        if (screen == Screen::PLAYING) 
        {
            ...
            // Helper: spawn a projectile from (ox,oy) toward (tx,ty)
            auto spawnProj = [&](float ox, float oy, float tx, float ty,
                sf::Color col) 
                {
            ...
            for (int l = 0; l < 3; l++) 
            {
                ...
                for (auto& u : targets) 
                {
                    if (std::abs(u.x - 400.f) <= LANE_T_RANGE_C) 
                    {
                        ...
                        laneTurrCD[l] = 1.f; // 1 / LANE_T_RATE(1.0)
                        break;
                    }
                }
            }
            ...
            // Main base turrets
            auto fireMainTurr = [&](float turrX, int level,
                bool isMine, float& cd) {
                    if (level == 0) return;
        ...
        else if (screen == Screen::WAITING || screen == Screen::CONNECTING) 
        {
            ...
            // Side base panels
            // Left panel = P1's base, right = P2's. We colour MY side blue, ENEMY red.
            ...
            // Vertical HP bars on inner edge of each panel
            {
                ...
                // Left panel bar — my side if P1, enemy side if P2
            ...
            // Perspective: left panel is MY side if P1, enemy side if P2
            ...
            for (int l = 0; l < 3; l++) 
            {
                ...
                bool neutral = !iOwn && !theyOwn;
                // Always: MY owned = blue ring, ENEMY owned = red ring
                ...
                // Capture progress bar — fill colour matches whichever team is ahead from MY perspective
                float fill = (g.baseCapture[l] + 100.f) / 200.f; // 1.0=P1 dominant, 0.0=P2
                float myFill = iAmP1 ? fill : (1.f - fill);
                sf::Color barFillCol = iOwn ? MY_TEAM : (theyOwn ? EN_TEAM : sf::Color(80, 85, 115));
                drawBar(window, 368.f, LANE_Y[l] + 21.f, 64.f, 5.f, myFill,
                    sf::Color(35, 20, 20), barFillCol);

                // Turret icon at capture point (when owned)
                // Barrel always fires toward the enemy's main base (server-side logic is fixed)
                ...
                if (!neutral) 
                {
                    ...
                    // Barrel points toward enemy territory (P1 fires right, P2 fires left)
            ...
            for (auto& p : projectiles) 
            {
                // Outer glow circle (fades as lifetime expires)
                float alpha = std::clamp(p.life / 0.5f, 0.f, 1.f);
                sf::Color glowCol = p.col;
                glowCol.a = (sf::Uint8)(80 * alpha);
                ...
                sf::Color coreCol = p.col;
                coreCol.a = (sf::Uint8)(230 * alpha);
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                // Cooldown overlay (dark block shrinks top - bottom)
                if (onCD && !locked)
                {
